...
func TestPopCountUseBitClear(t *testing.T) {
	for _, testCase := range testCases {
		result := PopCountUseBitClear(testCase.Input)
		if testCase.Output != result {
			t.Errorf("invalid result. testCaes:%v, actual:%v", testCase, result)
		}
	}
...
func BenchmarkPopCountUseBitClear(b *testing.B) {
	for i := 0; i < b.N; i++ {
		PopCountUseBitClear(testCases[0].Input)
	}
}
